...
fn solve(path: &str) -> io::Result<usize> {
    ...
    let dist = |(x, y): (usize, usize)| -> usize {
        ...
        for r in 0..row {
            for c in 0..col {
                if map[r][c] == galaxy {
                    let d = ((y + acc_y[y]) as isize - (r + acc_y[r]) as isize).abs()
                        + ((x + acc_x[x]) as isize - (c + acc_x[c]) as isize).abs();
...
fn main() {
    println!("{}", solve("./input/day11_1.txt").unwrap());
}
...
mod tests {
    ...
    #[test]
    fn sample() {
        assert_eq!(solve("./input/day11_1.sample.txt").unwrap(), 374);
    }
}
