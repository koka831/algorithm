...
fn solve(path: &str) -> io::Result<usize> {
    ...
    while let Some(Ok(line)) = lines.next() {
        let parts = line
            ...
            .split(',')
            .map(|rate| {
                let rate = rate.split_once('=').unwrap();
                (rate.0.to_string(), rate.1.parse::<usize>().unwrap())
            })
...
mod tests {
    ...
    #[test]
    fn sample() {
        assert_eq!(solve("./input/day19.sample.txt").unwrap(), 288);
    }
}
