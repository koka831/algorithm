...
impl FromStr for Hand {
    ...
    fn from_str(s: &str) -> Result<Self, Self::Err> {
        ...
        let hand = match pairs.as_slice() {
            [(_, 5)] => Hand::Five(s),
            [(_, 4), (x, 1)] if *x == 1 => Hand::Five(s),
            [(_, 4), (_, 1)] => Hand::Four(s),
            [(_, 3), (x, 2)] if *x == 1 => Hand::Five(s),
            [(_, 3), (_, 2)] => Hand::FullHouse(s),
            [(_, 3), (x, 1), (y, 1)] if *x == 1 || *y == 1 => Hand::Four(s),
            [(_, 3), (_, 1), (_, 1)] => Hand::Three(s),
            [(_, 2), (x, 2), (_, 1)] if *x == 1 => Hand::Four(s),
            [(_, 2), (_, 2), (y, 1)] if *y == 1 => Hand::FullHouse(s),
            [(_, 2), (_, 2), (_, 1)] => Hand::Two(s),
            [(_, 2), (x, 1), (y, 1), (z, 1)] if *x == 1 || *y == 1 || *z == 1 => Hand::Three(s),
            [(_, 2), (_, 1), (_, 1), (_, 1)] => Hand::One(s),
            [(_, 1), (_, 1), (_, 1), (_, 1), (_, 1)] if s.contains('J') => Hand::One(s),
...
fn main() {
    ...
    dbg!(Hand::from_str("JKKK2").cmp(&Hand::from_str("QQQQ2")));
}
...
mod tests {
    ...
    #[test]
    fn sample() {
        assert_eq!(solve("./input/day7_1.sample.txt").unwrap(), 5905);
    }
}
